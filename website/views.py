...
@extend_schema(
    ...
    description="Возвращает список опубликованных статей с возможностью фильтрации по категориям и рекомендуемым статьям",
    tags=["Insights"],
    parameters=[
        OpenApiParameter(
            name='category',
...
def website_data(request):
    ...
    try:
        ...
        about_data = AboutSectionSerializer(about).data if about else None
        ...
        insights_data = InsightListSerializer(featured_insights, many=True).data
        
        # Контактная информация
        contact_info = ContactInfo.objects.filter(is_active=True).first()
        contact_data = ContactInfoSerializer(contact_info).data if contact_info else None
